...
from pyfix.clock import set_manual_clock
...
class FIXCodecTests(unittest.TestCase):
    ...
    def setUp(self):
        set_manual_clock(datetime.datetime(2015, 6, 19, 11, 8, 54, tzinfo=datetime.timezone.utc))

    def testMinimalMessage(self):
        inMsg = b'8=FIX.4.4\x019=22\x0110=033\x018=FIX.4.4\x019=12\x0110=033\x01'
        SOH = b'\x01'
        s = inMsg.find(SOH)
        print(s)
        s2 = inMsg.find(SOH, s + 1)
        print(s2)
        tag, value =inMsg[s + 1 :s2].split(b'=')
        print(tag, value)
        if tag == b'9':
            print('jay')
        length = int(value)
    ...
    def _testDecode(self):
        protocol = LoadProtocol("pyfix.FIX44")
